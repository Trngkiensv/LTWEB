...
<section class="section-compact">
	<div class="container">
		...
		<div class="alert alert-success" role="alert">Đặt hàng thành
			công.!</div>
		<%
		} else if ("UPLOAD_SUCCESS".equals(msg)) {
		%>
		<div class="alert alert-success" role="alert">Chữ ký và/hoặc
			khóa công khai đã được tải lên thành công!</div>
		<%
		} else if ("UPLOAD_FAILED".equals(msg)) {
		%>
		<div class="alert alert-danger" role="alert">Lỗi tải lên chữ ký
			hoặc khóa công khai. Vui lòng thử lại!</div>
		<%
		} else if ("SIGNATURE_INVALID".equals(msg)) {
		%>
		<div class="alert alert-danger" role="alert">Chữ ký không hợp
			lệ. Vui lòng kiểm tra và thử lại!</div>
		<%
		} else if ("INVALID_PUBLIC_KEY".equals(msg)) {
		%>
		<div class="alert alert-danger" role="alert">Khóa công khai
			không hợp lệ. Vui lòng kiểm tra và thử lại!</div>
		<%
		}
		}
		%>
						...
						<tbody>
							...
							<tr>
								<th scope="row" class="text-center"><a idOrder=<%=idOrder%>
									title="" class="btn btn-danger del-order"><i
				...
				<div class="col-sm-7 col-lg-8">
					<div class="form-section-box">
						...
						<form class="checkout-form" method="post"
							...
							<div class="form-group row">
								...
								<div class="col-md-10">
									<input type="hidden" value="<%=userInfor.getId()%>"
										class="form-control" name="userID"> <input type="text"
										value="<%=userInfor.getFullname()%>" class="form-control"
							...
							<div class="form-group row">
								<div class="col-md-6">
									<label for="signatureFile">Chữ ký của bạn:</label> <input
										type="file" class="form-control-file" id="signatureFile"
										name="signatureFile" required>
								</div>
								<div class="col-md-6">
									<label for="publicKeyFile">Khóa công khai:</label> <input
										type="file" class="form-control-file" id="publicKeyFile"
										name="publicKeyFile" required>
								</div>
							</div>
							<div class="form-group row">
								<div class="col-md-6">
									<button type="submit" class="btn btn-info form-control">Thanh
										toán COD</button>
								</div>
								<div class="col-md-6">
									<button type="submit" class="btn btn-success form-control">Thanh
										toán Online</button>
								</div>
							</div>
						</form>
						...
						<div class="form-section-box mt-4">
							<header class="heading-5 text-center">
								<h4>03/</h4>
								<span>Chữ ký và khóa công khai</span>
							</header>
							<div class="row">
								<div class="col-md-6">
									<!-- Nút tải xuống chữ ký -->
									<a href="<%=request.getContextPath()%>/download-signature"
										class="btn btn-primary form-control">Tải xuống đơn hàng</a>
								</div>
							</div>
						</div>
